...
socket_depth           = 10.0; // Depth of sleeve wrapping around 2020 end (20mm)
...
partition_thick        = 2.5;  // Divider thickness between 2020 end and rod (5.0mm)

m5_bolt_diam           = 5.1;  // Center hole diameter for M5 bolt into 2020 end (5.3mm)
m3_tslot_setscrew_diam = 0.0;  // Set-screw hole to clamp 2020 extrusion (3.0mm)
...
m4_cs_depth            = 6.0;  // Countersink depth from outer cylinder surface (6.0mm)
...
difference() {
    ...
    // 6. Bottom Side Clamping Set-Screw Hole (M3 - Clamps 2020 T-Slot)
    translate([0, 0, -total_length/2 + (socket_depth / 2)])
        rotate([0, 90, 0])
            cylinder(d = m3_tslot_setscrew_diam, h = outer_diam/2 + 2, center = false);
}